...
export const CompanyPageMenu = ({ idEmpresa }) => {
  ...
  const handleBuyProduct = async (
    ...
    if (store.current_user_data.role == "Empresa") {
      toast.error("Must be a client to buy!", {
        position: "bottom-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
      ...
      toast.error("Must be logged to buy!", {
        position: "bottom-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
        ...
        <div className="row row-cols-2 d-flex justify-content-evenly">
          {products.length > 0 ? (
            products.map((product, index) => {
              return (
                <div
                  className="col-11 col-md-5 d-flex border menu_container_page"
                  key={index}
                >
